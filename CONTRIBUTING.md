...
- *IntelliJ* - IntelliJ idea can import and build gradle-based projects out of the box. However please note that it will default to running tests by calling the gradle wrapper, and while this works, it is for some reason quite slow. Instead we recommend configuring IntelliJ to use its own built-in test runner. You can modify this config (in 2024 version) by navigating to File/Settings/Build Execution & Deployment/Build Tools/Gradle and selecting "Build and Run using: IntelliJ IDEA" and "Run Tests using: IntelliJ IDEA".
