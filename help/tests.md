...
Examples below assume cwd at the gradlew script in the top directory of the project's checkout.
...
(\*) This step may omit certain validation tasks (errorprone, for example) that are slow or require external
resources. A pull request runs all those locally omitted tasks on the CI (jenkins, gh actions). For this reason, it's
a good idea to run patches via the CI.
...
Tests can be filtered by an annotation they're marked with. Some test group annotations include: `@AwaitsFix`,
`@Nightly`
...
Tests tasks will be (by default) re-executed on each invocation because we pick a random global `tests.seed` and
...
The `tests.iters` option should be sufficient for individual test cases and is *much* faster than trying to duplicate
...
Note the filter (`--tests`) used to narrow down test reiterations to a particular class. You can use any filter,
including no filter at all, but it rarely makes sense (will take ages). By default the test tasks generated by the
...
GUI test for Luke application launches a window, this might mess up your monitor depending on the display manager you
...
The `tests.profile` mode switch turns on a sampling profiler during test execution, and prints a simple summary at the
end.
...
By default, results are computed (deduplicated) on just the method name, folding together all events from the same
...
Running the `coverage` task (or setting the property `tests.coverage` to true) will run the tests with instrumentation
to record code coverage.
