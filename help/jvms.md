...
To run tests against a different Java version, define a property called `runtime.java.home` or define an environment
variable `RUNTIME_JAVA_HOME` pointing at the JDK installation folder.
...
Note that an alternative JVM can also be made the "default" setting by adding it to (project-local)
`gradle.properties`.
