  ...
  final MemorySegment getNativeSegment(int ord) throws IOException {
    ...
    if (seg == null) {
      if (offHeapScratch == null) {
        ...
        offHeapScratch = offHeap.allocate(vectorByteSize, ValueLayout.JAVA_BYTE.byteAlignment());
      }
  ...
  static final class NativeDotProductScorer extends Lucene99MemorySegmentByteVectorScorer {
    ...
    NativeDotProductScorer(
      ...
      if (offHeapQuery == null) {
        Arena offHeap = Arena.ofAuto();
        offHeapQuery = offHeap.allocate(vectorByteSize, ValueLayout.JAVA_BYTE.byteAlignment());
      }
    ...
    public float score(int node) throws IOException {
      ...
      float raw = PanamaVectorUtilSupport.nativeDotProduct(offHeapQuery, getNativeSegment(node));
