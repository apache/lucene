...
public final class SegmentInfos implements Cloneable, Iterable<SegmentCommitInfo> {
  ...
  static final SegmentInfos readCommit(
    ...
    try {
      ...
      int magic = CodecUtil.readInt(input);
  ...
  private static void parseSegmentInfos(
    ...
    infos.version = CodecUtil.readLong(input);
    // System.out.println("READ sis version=" + infos.version);
    if (format > VERSION_70) {
      infos.counter = input.readVLong();
    } else {
      infos.counter = CodecUtil.readInt(input);
    }
    int numSegments = CodecUtil.readInt(input);
    if (numSegments < 0) {
    ...
    for (int seg = 0; seg < numSegments; seg++) {
      ...
      long delGen = CodecUtil.readLong(input);
      int delCount = CodecUtil.readInt(input);
      if (delCount < 0 || delCount > info.maxDoc()) {
        throw new CorruptIndexException(
            "invalid deletion count: " + delCount + " vs maxDoc=" + info.maxDoc(), input);
      }
      long fieldInfosGen = CodecUtil.readLong(input);
      long dvGen = CodecUtil.readLong(input);
      int softDelCount = format > VERSION_72 ? CodecUtil.readInt(input) : 0;
      ...
      final int numDVFields = CodecUtil.readInt(input);
      if (numDVFields == 0) {
        dvUpdateFiles = Collections.emptyMap();
      } else {
        Map<Integer, Set<String>> map = new HashMap<>(numDVFields);
        for (int i = 0; i < numDVFields; i++) {
          map.put(CodecUtil.readInt(input), input.readSetOfStrings());
        }
  ...
  public void write(IndexOutput out) throws IOException {
    ...
    CodecUtil.writeLong(out, version);
    out.writeVLong(counter); // write counter
    CodecUtil.writeInt(out, size());
    ...
    for (SegmentCommitInfo siPerCommit : this) {
      ...
      CodecUtil.writeLong(out, siPerCommit.getDelGen());
      ...
      CodecUtil.writeInt(out, delCount);
      CodecUtil.writeLong(out, siPerCommit.getFieldInfosGen());
      CodecUtil.writeLong(out, siPerCommit.getDocValuesGen());
      ...
      CodecUtil.writeInt(out, softDelCount);
      ...
      CodecUtil.writeInt(out, dvUpdatesFiles.size());
      for (Entry<Integer, Set<String>> e : dvUpdatesFiles.entrySet()) {
        CodecUtil.writeInt(out, e.getKey());
