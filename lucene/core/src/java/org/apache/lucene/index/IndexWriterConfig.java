...
public final class IndexWriterConfig extends LiveIndexWriterConfig {
   ...
   * IndexWriter#addDocuments(Iterable)} or {@link IndexWriter#updateDocuments(Term, Iterable)}
   * (Iterable)} and it's relatives. Additionally, all individual documents added via the single
   * document methods ({@link IndexWriter#addDocuments(Iterable)} etc.) are also considered parent
   * documents. This property is optional for all indices that don't use document blocks in
   * combination with index sorting. In order to maintain the API guarantee that the document order
   * of a block is not altered by the {@link IndexWriter} a marker for parent documents is required.
   */
