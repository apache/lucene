...
import static org.apache.lucene.codecs.lucene105.ForUtil.BLOCK_SIZE;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.DOC_CODEC;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.LEVEL1_NUM_DOCS;
...
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.VERSION_CURRENT;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.VERSION_START;
 ...
 * @lucene.experimental
 */
public final class Lucene105PostingsReader extends PostingsReaderBase {
  ...
  /** Sole constructor. */
  public Lucene105PostingsReader(SegmentReadState state) throws IOException {
    String metaName =
  ...
  final class BlockPostingsEnum extends ImpactsEnum {
    ...
    public BlockPostingsEnum(FieldInfo fieldInfo, int flags, boolean needsImpacts)
      ...
      if (needsPos) {
        this.posIn = Lucene105PostingsReader.this.posIn.clone();
      ...
      if (needsOffsets || needsPayloads) {
        this.payIn = Lucene105PostingsReader.this.payIn.clone();
    ...
    public boolean canReuse(
      ...
      return docIn == Lucene105PostingsReader.this.docIn
          && options == fieldInfo.getIndexOptions()
          && indexHasPayloads == fieldInfo.hasPayloads()
          && this.flags == flags
          && this.needsImpacts == needsImpacts;
    ...
    public BlockPostingsEnum reset(IntBlockTermState termState, int flags) throws IOException {
      ...
      if (docFreq > 1) {
        if (docIn == null) {
          // lazy init
          docIn = Lucene105PostingsReader.this.docIn.clone();
      ...
      while (true) {
        ...
        if (indexHasFreq) {
          ...
          if (indexHasPos) {
            ...
            level1BlockPosUpto = docIn.readShort();
      ...
      if (docCountLeft >= BLOCK_SIZE) {
        ...
        if (indexHasFreq) {
          ...
          if (indexHasPos) {
            ...
            level0BlockPosUpto = docIn.readShort();
            if (indexHasOffsetsOrPayloads) {
    ...
    private final Impacts impacts =
        new Impacts() {
          ...
          private List<Impact> readImpacts(BytesRef serialized, MutableImpactList impactsList) {
            ...
            Lucene105PostingsReader.readImpacts(scratch, impactsList);
            return impactsList;
          }
        };
  ...
  /**
   * @see Lucene105PostingsWriter#writeVInt15(org.apache.lucene.store.DataOutput, int)
   */
  ...
  /**
   * @see Lucene105PostingsWriter#writeVLong15(org.apache.lucene.store.DataOutput, long)
   */
