...
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.BLOCK_SIZE;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.DOC_CODEC;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.LEVEL1_MASK;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.META_CODEC;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.PAY_CODEC;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.POS_CODEC;
import static org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.TERMS_CODEC;
...
import org.apache.lucene.codecs.lucene105.Lucene105PostingsFormat.IntBlockTermState;
...
/** Writer for {@link Lucene105PostingsFormat}. */
public class Lucene105PostingsWriter extends PushPostingsWriterBase {
  ...
  public Lucene105PostingsWriter(SegmentWriteState state) throws IOException {
    this(state, Lucene105PostingsFormat.VERSION_CURRENT);
  }

  /** Constructor that takes a version. */
  Lucene105PostingsWriter(SegmentWriteState state, int version) throws IOException {
    this.version = version;
    String metaFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name, state.segmentSuffix, Lucene105PostingsFormat.META_EXTENSION);
    String docFileName =
        IndexFileNames.segmentFileName(
            state.segmentInfo.name, state.segmentSuffix, Lucene105PostingsFormat.DOC_EXTENSION);
    ...
    try {
      ...
      if (state.fieldInfos.hasProx()) {
        ...
        String posFileName =
            IndexFileNames.segmentFileName(
                state.segmentInfo.name, state.segmentSuffix, Lucene105PostingsFormat.POS_EXTENSION);
        ...
        if (state.fieldInfos.hasPayloads() || state.fieldInfos.hasOffsets()) {
          String payFileName =
              IndexFileNames.segmentFileName(
                  ...
                  state.segmentSuffix,
                  Lucene105PostingsFormat.PAY_EXTENSION);
    ...
    if (docBufferUpto < BLOCK_SIZE) {
      ...
      if (writeFreqs) {
        ...
        if (writePositions) {
          ...
          level0Output.writeShort((short) posBufferUpto);
      ...
      // docID - lastBlockDocID is at least 256, so it can never fit a single byte with a vint
      // Even if we subtracted 256, only extremely dense blocks would be eligible to a single byte
  ...
  private void writeLevel1SkipData() throws IOException {
    ...
    if (writeFreqs) {
      ...
      if (writePositions) {
        ...
        scratchOutput.writeShort((short) posBufferUpto);
      ...
      // There are at most 256 impacts, that require at most 2 bytes each
