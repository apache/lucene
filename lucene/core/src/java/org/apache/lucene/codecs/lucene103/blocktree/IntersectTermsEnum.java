...
final class IntersectTermsEnum extends BaseTermsEnum {
  ...
  private int getState() {
    ...
    for (int idx = 0; idx < currentFrame.suffixLength; idx++) {
      state =
          runAutomaton.step(
  ...
  private void seekToStartTerm(BytesRef target) throws IOException {
    ...
    for (int idx = 0; idx <= target.length; idx++) {
      ...
      while (true) {
        ...
        final int saveSuffix = currentFrame.suffixLength;
        ...
        if (isSubBlock && StringHelper.startsWith(target, term)) {
          ...
          if (cmp < 0) {
            ...
            currentFrame.suffixLength = saveSuffix;
      ...
      if (currentFrame.suffixLength != 0) {
        ...
        if (commonSuffix != null && !isSubBlock) {
          ...
          if (lenInPrefix > 0) {
            ...
            suffixBytesPos = currentFrame.startBytePos + currentFrame.suffixLength - commonSuffix.length;
          }
