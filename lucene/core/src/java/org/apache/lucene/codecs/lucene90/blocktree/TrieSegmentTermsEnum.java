...
final class TrieSegmentTermsEnum extends BaseTermsEnum {

  // Lazy init:
  IndexInput in;
  private TrieSegmentTermsEnumFrame[] stack = new TrieSegmentTermsEnumFrame[0];
  private final TrieSegmentTermsEnumFrame staticFrame;
  TrieSegmentTermsEnumFrame currentFrame;
  boolean termExists;
...
//      stats.indexNumBytes = fr.trieReader.ramBytesUsed();
//    }
//
//    currentFrame = staticFrame;
//    TrieReader.Node node;
//    if (fr.trieReader != null) {
//      node = fr.trieReader.getFirstNode(nodes[0]);
...
//    // index!
//    currentFrame = pushFrame(node, fr.rootCode, 0);
...
//  }
  ...
  private TrieSegmentTermsEnumFrame getFrame(int ord) throws IOException {
    if (ord >= stack.length) {
      final TrieSegmentTermsEnumFrame[] next =
          new TrieSegmentTermsEnumFrame
              [ArrayUtil.oversize(1 + ord, RamUsageEstimator.NUM_BYTES_OBJECT_REF)];
      System.arraycopy(stack, 0, next, 0, stack.length);
      for (int stackOrd = stack.length; stackOrd < next.length; stackOrd++) {
        next[stackOrd] = new TrieSegmentTermsEnumFrame(this, stackOrd);
      }
      stack = next;
    }
  ...
  TrieSegmentTermsEnumFrame pushFrame(TrieReader.Node node, int length) throws IOException {
    ...
    final TrieSegmentTermsEnumFrame f = getFrame(1 + currentFrame.ord);
    ...
    if (f.isFloor) {
      f.setFloorData(output);
    }
    pushFrame(node, fpSeek, length);
  ...
  TrieSegmentTermsEnumFrame pushFrame(TrieReader.Node node, long fp, int length) throws IOException {
    final TrieSegmentTermsEnumFrame f = getFrame(1 + currentFrame.ord);
    f.node = node;
  ...
  private IOBooleanSupplier prepareSeekExact(BytesRef target, boolean prefetch) throws IOException {
    ...
    if (currentFrame != staticFrame) {
      ...
      TrieSegmentTermsEnumFrame lastFrame = stack[0];
      ...
      while (targetUpto < targetLimit) {
        ...
        // " vs termLabel=" + (char) (term.bytes[targetUpto]) + ")"   + " node.output=" + node.output
        // + " output=" + output);
        // }
        if (cmp != 0) {
          break;
  ...
  public SeekStatus seekCeil(BytesRef target) throws IOException {
    ...
    if (currentFrame != staticFrame) {
      ...
      while (targetUpto < targetLimit) {
        ...
        // System.out.println("    cycle targetUpto=" + targetUpto + " (vs limit=" + targetLimit +
        // ") cmp=" + cmp + " (targetLabel=" + (char) (target.bytes[target.offset + targetUpto]) +
        // " vs termLabel=" + (char) (term.byteAt(targetUpto)) + ")"   + " node.output=" + node.output
        // + " output=" + output);
        // }
        if (cmp != 0) {
          break;
