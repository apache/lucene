 ...
 * @lucene.experimental
 */
final class Lucene100CompoundReader extends CompoundDirectory {
  ...
  public Lucene100CompoundReader(Directory directory, SegmentInfo si, IOContext context)
      throws IOException {
    this.directory = directory;
    this.segmentName = si.name;
    String dataFileName =
        IndexFileNames.segmentFileName(segmentName, "", Lucene100CompoundFormat.DATA_EXTENSION);
    String entriesFileName =
        IndexFileNames.segmentFileName(segmentName, "", Lucene100CompoundFormat.ENTRIES_EXTENSION);
    ...
    final long expectedLength =
        entries.values().stream()
                ...
                .max()
                .orElseGet(
                    () -> CodecUtil.indexHeaderLength(Lucene100CompoundFormat.DATA_CODEC, ""))
            + CodecUtil.footerLength();

    handle = directory.openInput(dataFileName, context);
    try {
      CodecUtil.checkIndexHeader(
          handle, Lucene100CompoundFormat.DATA_CODEC, version, version, si.getId(), "");
