...
final class PForUtil {
  ...
  void encode(int[] ints, DataOutput out) throws IOException {
    ...
    final byte[] exceptions = new byte[numExceptions * 3];
    if (numExceptions > 0) {
      int exceptionCount = 0;
      for (int i = 0; i < ForUtil.BLOCK_SIZE; ++i) {
        if (ints[i] > maxUnpatchedValue) {
          BitUtil.VH_LE_SHORT.set(exceptions, exceptionCount * 3, (short) i);
          exceptions[exceptionCount * 3 + 2] = (byte) (ints[i] >>> patchedBitsRequired);
    ...
    if (allEqual(ints) && maxBitsRequired <= 8) {
      for (int i = 0; i < numExceptions; ++i) {
        exceptions[3 * i + 2] =
            (byte) (Byte.toUnsignedLong(exceptions[3 * i + 2]) << patchedBitsRequired);
      }
  ...
  void decode(PostingDecodingUtil pdu, int[] ints) throws IOException {
    ...
    for (int i = 0; i < numExceptions; ++i) {
      ints[in.readShort()] |= Byte.toUnsignedLong(in.readByte()) << bitsPerValue;
    }
  }
  ...
  static void skip(DataInput in) throws IOException {
    ...
    if (bitsPerValue == 0) {
      in.readVLong();
      in.skipBytes(numExceptions * 3);
    } else {
      in.skipBytes(ForUtil.numBytes(bitsPerValue) + numExceptions * 3);
    }
  }
}
