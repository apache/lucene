...
/**
 * An extension of the ByteArrayDataInput to support reading the input
 * byte array in a circular manner. This means that if the position goes
 * beyond the byte array's length, the internal position is reinitialized to zero.
 */
public final class ByteArrayCircularDataInput extends ByteArrayDataInput {
    ...
    private <T extends Number> T readValueWithLoopBack(int typeSize,
                                                       valueGenFunc<T> superFn, valueGenFromBytesFunc<T> readFn) {
    ...
    public short readShort() {
        ...
        return readValueWithLoopBack(Short.BYTES,
                () -> super.readShort(),
                (arr) -> (short) BitUtil.VH_LE_SHORT.get(arr, 0));
    }

    @Override
    public int readInt() {

        return readValueWithLoopBack(Integer.BYTES,
                () -> super.readInt(),
                (arr) -> (int) BitUtil.VH_LE_INT.get(arr, 0));
    }

    @Override
    public long readLong() {

        return readValueWithLoopBack(Long.BYTES,
                () -> super.readLong(),
                (arr) -> (long) BitUtil.VH_LE_LONG.get(arr, 0));
    }
