...
public abstract class PointRangeQuery extends Query {
  ...
  protected PointRangeQuery(String field, byte[] lowerPoint, byte[] upperPoint, int numDims) {
    config = new PRQConfig(field, lowerPoint, upperPoint, numDims);
  }
  ...
  public void visit(QueryVisitor visitor) {
    if (visitor.acceptField(config.field())) {
  ...
  public byte[] getLowerPoint() {
    return config.lowerPoint().clone();
  }

  public byte[] getUpperPoint() {
    return config.upperPoint().clone();
  }

  @Override
  public final int hashCode() {
    return (31 ^ 5 * classHash() + config.hashCode());
  }
  ...
  private boolean equalsTo(PointRangeQuery other) {
    return this.config.equalsTo(other.config);
  }

  @Override
  public final String toString(String field) {
    final StringBuilder sb = new StringBuilder();
    if (config.field().equals(field) == false) {
      sb.append(config.field());
      sb.append(':');
    }
    ...
    for (int i = 0; i < config.numDims(); i++) {
      ...
      int startOffset = config.bytesPerDim() * i;
