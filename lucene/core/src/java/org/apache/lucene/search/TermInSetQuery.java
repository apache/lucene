...
public class TermInSetQuery extends Query implements Accountable {
  ...
  private static class WeightOrDocIdSet {
    ...
    WeightOrDocIdSet(Scorer scorer) {
      this.set = null;
      this.weight = null;
      this.scorer = scorer;
    }
  ...
  public Weight createWeight(IndexSearcher searcher, ScoreMode scoreMode, float boost)
    ...
    return new ConstantScoreWeight(this, boost) {
      ...
      private WeightOrDocIdSet rewrite(
        ...
        if (dvType == DocValuesType.SORTED || dvType == DocValuesType.SORTED_SET) {
          ...
          if (termData.size() > costThreshold) {
            ...
            return new WeightOrDocIdSet(docValuesScorer(reader));
          }
        ...
        for (BytesRef term = iterator.next(); term != null; term = iterator.next()) {
          ...
          if (termsEnum.seekExact(term)) {
            ...
            if (matchingTerms == null) {
              ...
              expectedAdvances += Math.min(docFreq, leadCost);
              if (expectedAdvances > costThreshold) {
                return new WeightOrDocIdSet(docValuesScorer(reader));
              }
      ...
      public BulkScorer bulkScorer(LeafReaderContext context) throws IOException {
        final WeightOrDocIdSet weightOrBitSet =
            rewrite(context, Long.MAX_VALUE, false, DocValuesType.NONE);
        if (weightOrBitSet == null) {
          return null;
        } else if (weightOrBitSet.scorer != null) {
          return new DefaultBulkScorer(weightOrBitSet.scorer);
      ...
      public ScorerSupplier scorerSupplier(LeafReaderContext context) throws IOException {
        ...
        final DocValuesType dvType = reader.getFieldInfos().fieldInfo(field).getDocValuesType();

        final Weight weight = this;
        return new ScorerSupplier() {
          @Override
          public Scorer get(long leadCost) throws IOException {
            WeightOrDocIdSet weightOrDocIdSet =
                rewrite(context, leadCost, isPrimaryKeyField, dvType);
            final Scorer scorer;
            if (weightOrDocIdSet == null) {
              scorer = null;
            } else if (weightOrDocIdSet.scorer != null) {
              scorer = weightOrDocIdSet.scorer;
