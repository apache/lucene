...
class ConjunctionDISI extends DocIdSetIterator {
   ...
   */
  static DocIdSetIterator intersectSpans(List<Spans> spanList) {
    if (spanList.size() < 2) {
  ...
  static DocIdSetIterator createConjunction(
    ...
    List<DocIdSetIterator> iterators = new ArrayList<>(allIterators);
  ...
  private ConjunctionDISI(List<DocIdSetIterator> iterators) {
  ...
  private boolean assertItersOnSameDoc() {
    ...
    for (int i = 0; (i < others.length && iteratorsOnTheSameDoc); i++) {
      iteratorsOnTheSameDoc = others[i].docID() == curDoc;
    }
  ...
  private static final class ConjunctionTwoPhaseIterator extends TwoPhaseIterator {
    ...
    private ConjunctionTwoPhaseIterator(
      ...
      CollectionUtil.timSort(
          twoPhaseIterators,
          (Comparator<TwoPhaseIterator>) (o1, o2) -> Float.compare(o1.matchCost(), o2.matchCost()));
