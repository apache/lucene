...
abstract class AbstractKnnVectorQuery extends Query {
  ...
  private TopDocs searchLeaf(
      LeafReaderContext ctx, Weight filterWeight, MaxScoreAccumulator globalMinSimAcc)
      throws IOException {
    TopDocs results = getLeafResults(ctx, filterWeight, globalMinSimAcc);
    if (ctx.docBase > 0) {
  ...
  private TopDocs getLeafResults(
    ...
    if (filterWeight == null) {
      return approximateSearch(ctx, liveDocs, Integer.MAX_VALUE, globalMinSimAcc);
    }
    ...
    TopDocs results = approximateSearch(ctx, acceptDocs, cost, globalMinSimAcc);
