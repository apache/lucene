...
utf8Ranges = [(0, 127), (128, 2047), (2048, 65535), (65536, 1114111)]

typeToColor = {"startend": "purple", "start": "blue", "end": "red"}
...
class FSA:
        ...
        for b in bytes:
            ...
            for label, s, e, n in self.states[state][1:]:
                if b >= s and b <= e:
                    if found:
                        raise RuntimeError(
                            "state %s has ambiguous output for byte %s" % (oldState, b)
                        )
                    state = n
                    found = True
            if not found:
    ...
    def toDOT(self, label):
        ...
        w("  node [color=black];")
        w(
            '  node [shape=doublecircle, label=""]; N%s [label="%s"];'
            % (endNode, endNode)
        )
        w("  node [shape=circle];")
...
def getUTF8Rest(code, numBytes):
    ...
    for i in range(numBytes):
        rest.append((128 | (code & MASKS[5]), 6))
        code = code >> 6
    rest.reverse()
    return tuple(rest)
...
def start(fsa, startNode, endNode, utf8, doAll):
    if len(utf8) == 1:
        fsa.addEdge(
            startNode, endNode, utf8[0][0], utf8[0][0] | MASKS[utf8[0][1] - 1], "start"
        )
    else:
        n = fsa.addNode()
...
def end(fsa, startNode, endNode, utf8, doAll):
    if len(utf8) == 1:
        fsa.addEdge(
            startNode, endNode, utf8[0][0] & ~MASKS[utf8[0][1] - 1], utf8[0][0], "end"
        )
    else:
        if utf8[0][1] == 5:
...
def test(fsa, utf32Start, utf32End, count):
    ...
    for i in range(count):
        ...
        if dest != fsa.end:
            print(
                "FAILED: valid %s (%s) is not accepted"
                % (r, " ".join([binary(x[0]) for x in toUTF8(r)]))
            )
            return False
    ...
    if invalidRange >= 0:
        ...
        for i in range(count):
            ...
            if dest != -1:
                print(
                    "FAILED: invalid %s (%s) is accepted"
                    % (r, " ".join([binary(x[0]) for x in toUTF8(r)]))
                )
                return False
