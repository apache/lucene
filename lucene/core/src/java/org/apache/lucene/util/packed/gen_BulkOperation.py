...
def block_value_count(bpv, bits=64):
    ...
    while blocks % 2 == 0 and values % 2 == 0:
        blocks //= 2
        values //= 2
    assert values * bpv == bits * blocks, "%d values, %d blocks, %d bits per value" % (
        values,
        blocks,
        bpv,
    )
...
def p64_decode(bpv, f, bits):
    ...
    f.write("  @Override\n")
    f.write(
        "  public void decode(long[] blocks, int blocksOffset, %s[] values, int valuesOffset, int iterations) {\n"
        % typ
    )
    if bits < bpv:
        ...
        if is_power_of_two(bpv):
            ...
            f.write(
                "      for (int shift = %d; shift >= 0; shift -= %d) {\n"
                % (64 - bpv, bpv)
            )
            f.write(
                "        values[valuesOffset++] = %s(block >>> shift) & %d%s;\n"
            ...
            for i in range(0, values):
                ...
                if bit_offset == 0:
                    # start of block
                    f.write(
                        "      final long block%d = blocks[blocksOffset++];\n"
                        % block_offset
                    )
                    f.write(
                        "      values[valuesOffset++] = %sblock%d >>> %d%s;\n"
                        % (cast_start, block_offset, 64 - bpv, cast_end)
                    )
                elif bit_offset + bpv == 64:
                    # end of block
                    f.write(
                        "      values[valuesOffset++] = %sblock%d & %dL%s;\n"
                        % (cast_start, block_offset, mask, cast_end)
                    )
                elif bit_offset + bpv < 64:
                    # middle of block
                    f.write(
                        "      values[valuesOffset++] = %s(block%d >>> %d) & %dL%s;\n"
                        % (
                            cast_start,
                            block_offset,
                            64 - bit_offset - bpv,
                    ...
                    f.write(
                        "      values[valuesOffset++] = %s((block%d & %dL) << %d) | (block%d >>> %d)%s;\n"
                        % (
                            cast_start,
                            block_offset,
                            mask1,
    ...
    f.write("  @Override\n")
    f.write(
        "  public void decode(byte[] blocks, int blocksOffset, %s[] values, int valuesOffset, int iterations) {\n"
        % typ
    )
    if bits < bpv:
        f.write("    throw new UnsupportedOperationException();\n")
        ...
        if is_power_of_two(bpv) and bpv < 8:
            ...
            for i in range(0, byte_values):
                ...
                def shift(b):
                    return 8 * (byte_end - b - 1) + 1 + bit_end

                if bit_start == 0:
                    f.write(
                        "      final %s byte%d = blocks[blocksOffset++] & 0xFF;\n"
                        % (typ, byte_start)
                    )
                ...
                if byte_start == byte_end:
                    if bit_start == 0:
                        ...
                        if bit_end == 7:
                            f.write(
                                " byte%d & %d" % (byte_start, 2 ** (8 - bit_start) - 1)
                            )
                        else:
                            f.write(
                                " (byte%d >>> %d) & %d"
                    ...
                    if bit_start == 0:
                        ...
                        f.write(
                            " ((byte%d & %d) << %d)"
                            % (byte_start, 2 ** (8 - bit_start) - 1, shift(byte_start))
                        )
 ...
 */\n""")
        f2.write(
            "final class BulkOperationPacked%d extends BulkOperationPacked {\n" % bpv
        )
        packed64(bpv, f2)
        f2.write("}\n")
        f2.close()
