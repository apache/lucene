...
public class TestTermRangeQuery extends LuceneTestCase {
  ...
  public void testSubBlock() throws Exception {
    ...
    Query query = TermRangeQuery.newStringRange("category", "reg", "rez", true, true);

    IndexSearcher searcher = newSearcher(reader);
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals(3, hits.length);

    query = TermRangeQuery.newStringRange("category", "te", "tec", true, true);
    searcher = newSearcher(reader);
    hits = searcher.search(query, 1000).scoreDocs;
  ...
  public void testDeepSubBlock() throws Exception {
    ...
    IndexReader reader = DirectoryReader.open(writer);
    Query query = TermRangeQuery.newStringRange("category", "reg", "res", true, true);
    IndexSearcher searcher = newSearcher(reader);
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
  ...
  private void initializeIndex(String[] values, Analyzer analyzer) throws IOException {
    IndexWriter writer =
        new IndexWriter(dir, newIndexWriterConfig(analyzer).setOpenMode(OpenMode.CREATE));
