...
public class TestFullPrecisionFloat16VectorSimilarityValuesSource extends LuceneTestCase {
  ...
  public void testFullPrecisionVectorSimilarityDVS() throws Exception {
    ...
    try (Directory dir = newDirectory()) {
        ...
        for (int j = 0; j < numSegments; j++) {
          for (int i = 0; i < numVectors; i++) {
            ...
            if (random().nextInt(100) < 30) {
              ...
              short[] vector = TestVectorUtil.randomVectorFloat16(VECTOR_DIMENSION);
              vectors.add(vector);
      ...
      short[] queryVector = TestVectorUtil.randomVectorFloat16(VECTOR_DIMENSION);
      ...
      try (IndexReader reader = DirectoryReader.open(dir)) {
        for (LeafReaderContext ctx : reader.leaves()) {
          ...
          if (fpSimValues == DoubleValues.EMPTY || quantizedSimValues == DoubleValues.EMPTY) {
            ...
            assertNull(ctx.reader().getFloat16VectorValues(KNN_FIELD));
            continue;
          }
          StoredFields storedFields = ctx.reader().storedFields();
          VectorScorer quantizedScorer =
              ctx.reader().getFloat16VectorValues(KNN_FIELD).scorer(queryVector);
