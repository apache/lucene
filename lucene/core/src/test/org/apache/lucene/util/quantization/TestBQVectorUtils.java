...
public class TestBQVectorUtils extends LuceneTestCase {
  ...
  public void testPadFloat() {
    assertArrayEquals(
        new float[] {1, 2, 3, 4}, BQVectorUtils.pad(new float[] {1, 2, 3, 4}, 4), DELTA);
    assertArrayEquals(
        new float[] {1, 2, 3, 4}, BQVectorUtils.pad(new float[] {1, 2, 3, 4}, 3), DELTA);
    assertArrayEquals(
        new float[] {1, 2, 3, 4, 0}, BQVectorUtils.pad(new float[] {1, 2, 3, 4}, 5), DELTA);
  }
  ...
  public void testPopCount() {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      var a = new byte[size];
      random().nextBytes(a);
      assertEquals(TestVectorUtil.popcount(a, 0, a, size), BQVectorUtils.popcount(a));
    }
  }
  ...
  public void testSubtract() {
    assertArrayEquals(
        new float[] {1}, BQVectorUtils.subtract(new float[] {3}, new float[] {2}), DELTA);
    assertArrayEquals(
        new float[] {2, 1, 0},
        BQVectorUtils.subtract(new float[] {3, 3, 3}, new float[] {1, 2, 3}),
        DELTA);
  }
