...
public class TestPerFieldKnnVectorsFormat extends BaseKnnVectorsFormatTestCase {
      ...
      try (IndexReader ireader = DirectoryReader.open(directory)) {
        ...
        TopDocs hits1 =
            reader.searchNearestVectors(
                "field1", new float[] {1, 2, 3}, 10, reader.getLiveDocs(), Integer.MAX_VALUE, HnswGraphSearcher.Multivalued.NONE);
        assertEquals(1, hits1.scoreDocs.length);

        TopDocs hits2 =
            reader.searchNearestVectors(
                "field2", new float[] {1, 2, 3}, 10, reader.getLiveDocs(), Integer.MAX_VALUE, HnswGraphSearcher.Multivalued.NONE);
        assertEquals(1, hits2.scoreDocs.length);
      }
    }
  }

  public void testTwoFieldsTwoFormats_multiValued() throws IOException {
    try (Directory directory = newDirectory()) {
      // we don't use RandomIndexWriter because it might add more values than we expect !!!!1
      IndexWriterConfig iwc = newIndexWriterConfig(new MockAnalyzer(random()));
      WriteRecordingKnnVectorsFormat format1 =
      ...
      try (IndexWriter iwriter = new IndexWriter(directory, iwc)) {
        Document doc = new Document();
        doc.add(newTextField("id", "1", Field.Store.YES));
        doc.add(new KnnVectorField("field1", new float[] {1, 2, 3}, true));
        doc.add(new KnnVectorField("field1", new float[] {1, 2, 3}, true));
        ...
        doc.clear();
        doc.add(newTextField("id", "2", Field.Store.YES));
        doc.add(new KnnVectorField("field2", new float[] {4, 5, 6}, true));
        doc.add(new KnnVectorField("field2", new float[] {4, 5, 6}, true));
        doc.add(new KnnVectorField("field2", new float[] {4, 5, 6}, true));
      ...
      try (IndexReader ireader = DirectoryReader.open(directory)) {
        ...
        TopDocs hits1 =
                reader.searchNearestVectors(
                        "field1", new float[] {1, 2, 3}, 10, reader.getLiveDocs(), Integer.MAX_VALUE, HnswGraphSearcher.Multivalued.MAX);
        assertEquals(1, hits1.scoreDocs.length);

        TopDocs hits2 =
                reader.searchNearestVectors(
                        "field2", new float[] {1, 2, 3}, 10, reader.getLiveDocs(), Integer.MAX_VALUE, HnswGraphSearcher.Multivalued.MAX);
        assertEquals(1, hits2.scoreDocs.length);
      }
    }
  }
  ...
  public void testMergeUsesNewFormat_multiValued() throws IOException {
    try (Directory directory = newDirectory()) {
      IndexWriterConfig initialConfig = newIndexWriterConfig(new MockAnalyzer(random()));
      initialConfig.setMergePolicy(NoMergePolicy.INSTANCE);
      ...
      try (IndexWriter iw = new IndexWriter(directory, initialConfig)) {
        for (int i = 0; i < 3; i++) {
          ...
          doc.add(new KnnVectorField("field1", new float[] {1, 2, 3}, true));
          doc.add(new KnnVectorField("field1", new float[] {1, 2, 3}, true));
          doc.add(new KnnVectorField("field2", new float[] {1, 2, 3}, true));
          doc.add(new KnnVectorField("field2", new float[] {1, 2, 3}, true));
          doc.add(new KnnVectorField("field2", new float[] {1, 2, 3}, true));
          iw.addDocument(doc);
          iw.commit();
