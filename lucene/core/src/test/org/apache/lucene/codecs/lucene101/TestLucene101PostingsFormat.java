...
public class TestLucene101PostingsFormat extends BasePostingsFormatTestCase {
    ...
    public static SegmentReadState fakeSegmentReadState(Directory dir, String segmentName, int maxDoc) throws IOException {
        ...
        try (Directory dir = newDirectory()) {
            ...
            for (String file : dir.listAll()) {
                if (file.startsWith(segmentName + "_") && file.endsWith(".binmap")) {
                    binmapFile = file;
                    segmentSuffix = file.substring(segmentName.length() + 1, file.length() - ".binmap".length());
                    break;
                }
            }
    ...
    public void testBinMapWithCompoundFileFormat() throws Exception {
        try (Directory dir = newDirectory()) {
            ...
            try (Directory cfsDir = info.info.getCodec().compoundFormat().getCompoundReader(dir, info.info)) {
                String binmapFile = null;
                String segmentSuffix = null;
    ...
    public void testBinMapBinningDisabled() throws Exception {
        try (Directory dir = newDirectory()) {
            ...
            for (String file : dir.listAll()) {
                assertFalse("binmap should not exist when binning is not enabled", file.endsWith(".binmap"));
            }
        }
    }
...
public void testBinMapFileSurvivesMerge() throws Exception {
  try (Directory dir = newDirectory()) {
    IndexWriterConfig iwc = new IndexWriterConfig(new StandardAnalyzer());
    iwc.setMaxBufferedDocs(2); // create multiple segments
    ...
    try (IndexWriter writer = new IndexWriter(dir, iwc)) {
      ...
      for (int i = 0; i < 20; i++) {
        Document doc = new Document();
        doc.add(new Field("content", "lucene merge test", ft));
        writer.addDocument(doc);
      }
    ...
    // Reopen the dir under TrackingDirectoryWrapper to inspect physical files
    try (TrackingDirectoryWrapper tracked = new TrackingDirectoryWrapper(dir)) {
      // Confirm exactly one .binmap file exists after merge
      try (DirectoryReader reader = DirectoryReader.open(tracked)) {
        ...
        long binCount = files.stream().filter(f -> f.endsWith(".binmap")).count();

        assertEquals("Merged segment should have one .binmap file", 1, binCount);
      }
    }
  }
}
