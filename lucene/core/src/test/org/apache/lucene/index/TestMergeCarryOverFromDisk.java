...
/**
 * Deterministically exercises the doc-values-update merge carry-over: an update that resolves onto
 * a segment <em>while it is being merged</em> must appear on the merged segment. With overlays
 * enabled the carry-over is reconstructed from the source segments at merge commit; with overlays
 * disabled (the default here) it comes from the in-heap buffer. The test blocks a merge mid-flight
 * (only its own output, via {@link IOContext#mergeInfo}, so the update's flush is free), applies +
 * resolves updates, then releases the merge. {@link #testFoldedOverlaysCarriedOverDuringMerge}
 * enables overlays and so drives the disk-reconstruction path.
 */
public class TestMergeCarryOverFromDisk extends LuceneTestCase {
  ...
  public void testUpdatesResolvedDuringMergeAreCarriedOver() throws Exception {
    doUpdatesResolvedDuringMerge(0);
  }

  public void testUpdatesResolvedDuringMergeAreCarriedOverWithOverlays() throws Exception {
    doUpdatesResolvedDuringMerge(4);
  }

  private void doUpdatesResolvedDuringMerge(int maxOverlays) throws Exception {
    MergePausingDirectory dir = new MergePausingDirectory(newDirectory());
    IndexWriterConfig conf =
        newIndexWriterConfig(new MockAnalyzer(random()))
            .setMergeScheduler(new ConcurrentMergeScheduler())
            .setMaxDocValuesOverlays(maxOverlays);
   ...
   * absent from the source reader. Run on both carry-over paths: the buffered path (overlays off)
   * and the disk-reconstruction path (overlays on).
   */
  public void testSoftDeleteResolvedDuringMergeIsCarriedOver() throws Exception {
    doSoftDeleteResolvedDuringMerge(0);
  }

  public void testSoftDeleteResolvedDuringMergeIsCarriedOverWithOverlays() throws Exception {
    doSoftDeleteResolvedDuringMerge(4);
  }

  private void doSoftDeleteResolvedDuringMerge(int maxOverlays) throws Exception {
    MergePausingDirectory dir = new MergePausingDirectory(newDirectory());
    IndexWriterConfig conf =
        newIndexWriterConfig(new MockAnalyzer(random()))
            .setMergeScheduler(new ConcurrentMergeScheduler())
