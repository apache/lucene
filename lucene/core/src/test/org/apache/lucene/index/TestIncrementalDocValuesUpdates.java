...
public class TestIncrementalDocValuesUpdates extends LuceneTestCase {
  ...
  /**
   * Exercises a high overlay budget under heavy stacking and repeated folding, which neither {@link
   * #newIndexWriterConfig} (small randomized budget) nor {@link #incrementalConfig} (low range)
   * reaches. Every update is resolved to its own generation via a reader reopen so overlays stack
   * up to the budget and fold, across two independent numeric fields, and every value must still
   * read back correctly. Occasional force-merges keep the segment (and open file handle) count
   * bounded so it also runs under the default open-handle limit.
   */
  public void testHighBudgetHeavyStackingAndFolding() throws Exception {
    IndexWriterConfig conf =
        new IndexWriterConfig(new MockAnalyzer(random())).setMaxDocValuesOverlays(16);
