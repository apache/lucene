...
public class TestReaderUtil extends LuceneTestCase {
  ...
  public void testPartitionByLeafSortsUnsortedInput() throws IOException {
    try (Directory dir = newDirectory();
        IndexWriter writer = new IndexWriter(dir, new IndexWriterConfig())) {
      for (int i = 0; i < 10; i++) {
        writer.addDocument(new Document());
      ...
      try (DirectoryReader reader = DirectoryReader.open(writer)) {
        ...
        // Input deliberately out of order; per-leaf output must be sorted.
        int[] docIds = {9, 3, 0, 5};
        int[][] result = ReaderUtil.partitionByLeaf(docIds, leaves);
  ...
  public void testPartitionByLeafDoesNotMutateInput() throws IOException {
    try (Directory dir = newDirectory();
        IndexWriter writer = new IndexWriter(dir, new IndexWriterConfig())) {
      for (int i = 0; i < 10; i++) {
        writer.addDocument(new Document());
  ...
  public void testPartitionByLeafWithOrdinalsEmpty() throws IOException {
    try (Directory dir = newDirectory();
        IndexWriter writer = new IndexWriter(dir, new IndexWriterConfig())) {
      writer.addDocument(new Document());
      try (DirectoryReader reader = DirectoryReader.open(writer)) {
        List<LeafReaderContext> leaves = reader.leaves();
        ReaderUtil.PartitionedHits partitioned =
            ReaderUtil.partitionByLeafWithOrdinals(new int[0], leaves);
  ...
  public void testPartitionByLeafWithOrdinalsTracksOriginalPositions() throws IOException {
    try (Directory dir = newDirectory();
        IndexWriter writer =
            new IndexWriter(dir, new IndexWriterConfig().setMergePolicy(NoMergePolicy.INSTANCE))) {
      for (int i = 0; i < 10; i++) {
      ...
      try (DirectoryReader reader = DirectoryReader.open(writer)) {
        ...
        ReaderUtil.PartitionedHits partitioned =
            ReaderUtil.partitionByLeafWithOrdinals(docIds, leaves);
  ...
  public void testPartitionByLeafWithOrdinalsDoesNotMutateInput() throws IOException {
    try (Directory dir = newDirectory();
        IndexWriter writer = new IndexWriter(dir, new IndexWriterConfig())) {
      for (int i = 0; i < 10; i++) {
        writer.addDocument(new Document());
  ...
  public void testPartitionByLeafWithOrdinalsRandomized() throws IOException {
    for (int iter = 0; iter < 50; iter++) {
      int numSegments = random().nextInt(8) + 1;
      int totalDocs = 0;
      int[] docsPerSegment = new int[numSegments];
      for (int i = 0; i < numSegments; i++) {
        docsPerSegment[i] = random().nextInt(80) + 1;
        totalDocs += docsPerSegment[i];
      }
