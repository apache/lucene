...
/**
 * An {@link Analyzer} used primarily at query time to wrap another analyzer and provide a layer of
 * protection which prevents very common words from being passed into queries.
 *
 * <p>For very large indexes the cost of reading TermDocs for a very common word can be high. This
 * analyzer was created after experience with a 38 million doc index which had a term in around 50%
 * of docs and was causing TermQueries for this term to take 2 seconds.
 *
 * @since 3.1
 */
public final class QueryAutoStopWordAnalyzer extends AnalyzerWrapper {
  ...
  /**
   * Creates a new QueryAutoStopWordAnalyzer with stopwords calculated for all indexed fields from
   * terms with a document frequency percentage greater than {@link #defaultMaxDocFreqPercent}
   *
  ...
  /**
   * Creates a new QueryAutoStopWordAnalyzer with stopwords calculated for all indexed fields from
   * terms with a document frequency greater than the given maxDocFreq
   *
   * @param delegate Analyzer whose TokenStream will be filtered
   * @param indexReader IndexReader to identify the stopwords from
   ...
   * @throws IOException Can be thrown while reading from the IndexReader
   */
  public QueryAutoStopWordAnalyzer(Analyzer delegate, IndexReader indexReader, int maxDocFreq)
      throws IOException {
  ...
  /**
   * Creates a new QueryAutoStopWordAnalyzer with stopwords calculated for all indexed fields from
   * terms with a document frequency percentage greater than the given maxPercentDocs
   *
   * @param delegate Analyzer whose TokenStream will be filtered
   * @param indexReader IndexReader to identify the stopwords from
   ...
   * @throws IOException Can be thrown while reading from the IndexReader
   */
  public QueryAutoStopWordAnalyzer(Analyzer delegate, IndexReader indexReader, float maxPercentDocs)
      throws IOException {
   ...
   * fields from terms with a document frequency percentage greater than the given maxPercentDocs
   *
   * @param delegate Analyzer whose TokenStream will be filtered
   * @param indexReader IndexReader to identify the stopwords from
  ...
  public QueryAutoStopWordAnalyzer(
      Analyzer delegate, IndexReader indexReader, Collection<String> fields, float maxPercentDocs)
      throws IOException {
  ...
  protected TokenStreamComponents wrapComponents(
    ...
    StopFilter stopFilter =
        new StopFilter(components.getTokenStream(), new CharArraySet(stopWords, false));
   ...
   *
   * @param fieldName The field for which stop words identified in "addStopWords" method calls will
   *     be returned
