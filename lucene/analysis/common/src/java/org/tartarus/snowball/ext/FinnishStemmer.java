 ...
 *
 * <p>Generated from finnish.sbl by Snowball 3.0.0 - https://snowballstem.org/
 */
...
public class FinnishStemmer extends org.tartarus.snowball.SnowballStemmer {
  ...
  private boolean r_mark_regions() {
    I_p1 = limit;
    I_p2 = limit;
    if (!go_out_grouping(g_V1, 97, 246)) {
      return false;
    }
    cursor++;
    if (!go_in_grouping(g_V1, 97, 246)) {
      return false;
    }
    cursor++;
    I_p1 = cursor;
  ...
  private boolean r_tidy() {
    ...
    cursor = limit - v_7;
    limit_backward = v_1;
    if (!go_in_grouping_b(g_V1, 97, 246)) {
      return false;
    }
    ket = cursor;
    if (!(in_grouping_b(g_C, 98, 122))) {
    ...
    bra = cursor;
    slice_check();
    if (S_x.length < ket - bra) {
      S_x = Arrays.copyOfRange(current, bra, ket);
    } else {
      System.arraycopy(current, bra, S_x, 0, ket - bra);
    }
    LS_x = ket - bra;
    if (!(eq_s_b(new CharArraySequence(S_x, LS_x)))) {
      return false;
    }
    slice_del();
    return true;
