...
/**
 * This directory wrapper overrides {@link Directory#copyFrom(Directory, String, String)} in order
 * to optionally use a hard-link instead of a full byte by byte file copy if applicable. Hard-links
 * are only used if the underlying filesystem supports it and if the {@link
 * java.nio.file.LinkPermission} "hard" is granted.
 *
 * <p><b>NOTE:</b> Using hard-links changes the copy semantics of {@link
 * Directory#copyFrom(Directory, String, String)}. When hard-links are used changes to the source
 * file will be reflected in the target file and vice-versa. Within Lucene, files are write once and
 * should not be modified after they have been written. This directory should not be used in
 * situations where files change after they have been written.
 */
public final class HardlinkCopyDirectoryWrapper extends FilterDirectory {
  ...
  @Override
  public void copyFrom(Directory from, String srcFile, String destFile) throws IOException {
    ...
    if (tryCopy) {
      try {
        getDelegate().copyFrom(from, srcFile, destFile);
      } catch (Exception ex) {
