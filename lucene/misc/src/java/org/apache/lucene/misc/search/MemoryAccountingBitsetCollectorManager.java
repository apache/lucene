...
public class MemoryAccountingBitsetCollectorManager
   ...
   *
   * <p>The returned {@link FixedBitSet} has length {@code highestMatchedDoc + 1}, or {@code 0} if
   * no document matched; it is not padded to the searched index range. Callers probing document ids
   * beyond the last match should first check {@link FixedBitSet#length()} before calling {@link
   * FixedBitSet#get(int)}.
   */
