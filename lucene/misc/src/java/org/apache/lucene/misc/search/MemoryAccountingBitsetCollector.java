...
public class MemoryAccountingBitsetCollector extends SimpleCollector {
  ...
  public void collect(int doc) {
    ...
    if (bitSet == null) {
      ...
      // Charge the tracker BEFORE publishing bitSet, so that if updateBytes throws
      // (memory limit exceeded), the field stays null and the invariant
      // (bitSet == null) == (highestSetBit == -1) is preserved unconditionally.
      FixedBitSet newBitSet = new FixedBitSet(local + 1);
      tracker.updateBytes(newBitSet.ramBytesUsed());
      bitSet = newBitSet;
    } else {
