...
class Geo3DUtil {
  ...
  static GeoPolygon fromLargePolygon(final PlanetModel planetModel, final Polygon... polygons) {
    ...
    return GeoPolygonFactory.makeLargeGeoPolygon(
        planetModel, convertToDescription(planetModel, polygons));
  }
  ...
  static GeoCircle fromDistance(
    ...
    return GeoCircleFactory.makeGeoCircle(
        planetModel, fromDegrees(latitude), fromDegrees(longitude), radiusRadians);
  }
  ...
  static GeoBBox fromBox(
    ...
    return GeoBBoxFactory.makeGeoBBox(
        planetModel,
        Geo3DUtil.fromDegrees(maxLatitude),
        Geo3DUtil.fromDegrees(minLatitude),
        Geo3DUtil.fromDegrees(minLongitude),
   ...
   */
  private static List<GeoPolygonFactory.PolygonDescription> convertToDescription(
      final PlanetModel planetModel, final Polygon... polygons) {
    final List<GeoPolygonFactory.PolygonDescription> descriptions =
        new ArrayList<>(polygons.length);
    for (final Polygon polygon : polygons) {
      final Polygon[] theHoles = polygon.getHoles();
      final List<GeoPolygonFactory.PolygonDescription> holes =
          convertToDescription(planetModel, theHoles);
