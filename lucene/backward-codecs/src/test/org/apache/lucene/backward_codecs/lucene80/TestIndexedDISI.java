...
public class TestIndexedDISI extends LuceneTestCase {
  ...
  private void assertAdvanceBeyondEnd(BitSet set, Directory dir) throws IOException {
    ...
    try (IndexOutput out = DirectoryUtil.createOutput(dir, "bar", IOContext.DEFAULT)) {
      jumpTableentryCount =
          IndexedDISI.writeBitSet(new BitSetIterator(set, cardinality), out, denseRankPower);
    }

    try (IndexInput in = DirectoryUtil.openInput(dir, "bar", IOContext.DEFAULT)) {
  ...
  public void testPositionNotZero() throws IOException {
    ...
    try (Directory dir = newDirectory()) {
      final int cardinality = set.cardinality();
      int jumpTableEntryCount;
      try (IndexOutput out = DirectoryUtil.createOutput(dir, "foo", IOContext.DEFAULT)) {
        jumpTableEntryCount =
            IndexedDISI.writeBitSet(new BitSetIterator(set, cardinality), out, denseRankPower);
      }
      try (IndexInput fullInput = DirectoryUtil.openInput(dir, "foo", IOContext.DEFAULT)) {
  ...
  private void doTestAllSingleJump(BitSet set, Directory dir) throws IOException {
    ...
    try (IndexInput in = DirectoryUtil.openInput(dir, "foo", IOContext.DEFAULT)) {
  ...
  public void testSparseDenseBoundary() throws IOException {
    try (Directory dir = newDirectory()) {
      ...
      long length;
      int jumpTableEntryCount;
      try (IndexOutput out = DirectoryUtil.createOutput(dir, "sparse", IOContext.DEFAULT)) {
        jumpTableEntryCount =
            IndexedDISI.writeBitSet(
                new BitSetIterator(set, IndexedDISI.MAX_ARRAY_LENGTH), out, denseRankPower);
        length = out.getFilePointer();
      }
      try (IndexInput in = DirectoryUtil.openInput(dir, "sparse", IOContext.DEFAULT)) {
        IndexedDISI disi =
            new IndexedDISI(
