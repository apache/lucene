...
public class TestLucene103PostingsFormat extends BasePostingsFormatTestCase {
  ...
  public void testVInt15() throws IOException {
    ...
    for (int i : new int[] {0, 1, 127, 128, 32767, 32768, Integer.MAX_VALUE}) {
      out.reset(bytes);
      Lucene103PostingsWriter.writeVInt15(out, i);
      in.reset(bytes, 0, out.getPosition());
      assertEquals(i, Lucene103PostingsReader.readVInt15(in));
  ...
  public void testVLong15() throws IOException {
    ...
    for (long i : new long[] {0, 1, 127, 128, 32767, 32768, Integer.MAX_VALUE, Long.MAX_VALUE}) {
      out.reset(bytes);
      Lucene103PostingsWriter.writeVLong15(out, i);
      in.reset(bytes, 0, out.getPosition());
      assertEquals(i, Lucene103PostingsReader.readVLong15(in));
      assertEquals(out.getPosition(), in.getPosition());
    }
  }
  ...
  private void doTestImpactSerialization(List<Impact> impacts) throws IOException {
    ...
    try (Directory dir = newDirectory()) {
      ...
      try (IndexInput in = dir.openInput("foo", IOContext.DEFAULT)) {
        ...
        impactBuffer = Lucene103PostingsReader.readImpacts(new ByteArrayDataInput(b), impactBuffer);
