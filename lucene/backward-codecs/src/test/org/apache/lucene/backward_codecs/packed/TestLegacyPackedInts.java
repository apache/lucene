...
public class TestLegacyPackedInts extends LuceneTestCase {
  ...
  public void testPackedInts() throws IOException {
    ...
    for (int iter = 0; iter < num; iter++) {
      for (int nbits = 1; nbits <= 64; nbits++) {
        ...
        IndexOutput out = DirectoryUtil.createOutput(d, "out.bin", newIOContext(random()));
        ...
        { // test reader
          IndexInput in = DirectoryUtil.openInput(d, "out.bin", newIOContext(random()));
          PackedInts.Reader r =
  ...
  public void testEndPointer() throws IOException {
    ...
    final IndexOutput out = DirectoryUtil.createOutput(dir, "tests.bin", newIOContext(random()));
    for (int i = 0; i < valueCount; ++i) {
      out.writeLong(0);
    }
    out.close();
    final IndexInput in = DirectoryUtil.openInput(dir, "tests.bin", newIOContext(random()));
  ...
  public void testSingleValue() throws Exception {
    for (int bitsPerValue = 1; bitsPerValue <= 64; ++bitsPerValue) {
      ...
      IndexOutput out = DirectoryUtil.createOutput(dir, "out", newIOContext(random()));
      PackedInts.Writer w =
      ...
      IndexInput in = DirectoryUtil.openInput(dir, "out", newIOContext(random()));
      Reader reader =
          LegacyPackedInts.getReaderNoHeader(
