...
public class TestLegacyPackedInts extends LuceneTestCase {
  ...
  public void testPackedInts() throws IOException {
    ...
    for (int iter = 0; iter < num; iter++) {
      for (int nbits = 1; nbits <= 64; nbits++) {
        ...
        IndexOutput out =
            EndiannessReverserUtil.createOutput(d, "out.bin", newWriteIOContext(random()));
        ...
        { // test reader
          IndexInput in =
              EndiannessReverserUtil.openInput(d, "out.bin", newReadIOContext(random()));
          PackedInts.Reader r =
  ...
  public void testEndPointer() throws IOException {
    ...
    final IndexOutput out =
        EndiannessReverserUtil.createOutput(dir, "tests.bin", newWriteIOContext(random()));
    for (int i = 0; i < valueCount; ++i) {
      out.writeLong(0);
    }
    out.close();
    final IndexInput in =
        EndiannessReverserUtil.openInput(dir, "tests.bin", newReadIOContext(random()));
  ...
  public void testSingleValue() throws Exception {
    for (int bitsPerValue = 1; bitsPerValue <= 64; ++bitsPerValue) {
      ...
      IndexOutput out =
          EndiannessReverserUtil.createOutput(dir, "out", newWriteIOContext(random()));
      PackedInts.Writer w =
      ...
      IndexInput in = EndiannessReverserUtil.openInput(dir, "out", newReadIOContext(random()));
      Reader reader =
          LegacyPackedInts.getReaderNoHeader(
