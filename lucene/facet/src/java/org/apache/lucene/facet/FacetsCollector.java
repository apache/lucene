...
public class FacetsCollector extends SimpleCollector {
  ...
  protected static Collection<MatchingDocs> reduceMatchingDocs(
    ...
    // When a segment is split into partitions, each partition gets its own FacersCollector that
    // pulls doc_values
    // independently, and build a bitset of the size of the entire segment. Each partition will
    // collect only the
    // docs in its docid range, hence there will be multiple MatchingDocs pointing to the same
    // LeafReaderContext.
    // As part of the reduction we merge back partitions into a single MatchingDocs per segment.
