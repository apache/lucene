...
public class SortedSetDocValuesFacetCounts extends AbstractSortedSetDocValueFacetCounts {
  ...
  public FacetResult getAllChildren(String dim, String... path) throws IOException {
    ...
    if (dimConfig.hierarchical) {
      ...
      FacetResult facetResult = getPathResult(dimConfig, dim, null, dimOrd, childIt);
      return facetResult;
    }
  }
  ...
  private FacetResult getPathResult(
    ...
    while (childOrds.hasNext()) {
      ...
      if (counts[ord] > 0) {
        ...
        labelValues.add(new LabelAndValue(parts[parts.length - 1], counts[ord]));
      }
    }

    if (dimConfig.hierarchical == true) {
      return new FacetResult(
          dim, path, counts[pathOrd], labelValues.toArray(new LabelAndValue[0]), childCount);
    } else {
