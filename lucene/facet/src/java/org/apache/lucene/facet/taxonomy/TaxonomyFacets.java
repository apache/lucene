...
abstract class TaxonomyFacets extends Facets {
  ...
  private FacetResult createFacetResult(
    ...
    for (int i = 0; i < labelValues.length; i++) {
      labelValues[i] = new LabelAndValue(bulkPath[i].components[childComponentIdx], values[i], getCount(ordinals[i]));
    }
  ...
  public FacetResult getAllChildren(String dim, String... path) throws IOException {
    ...
    for (int i = 0; i < ordValues.size(); i++) {
      labelValues[i] = new LabelAndValue(bulkPath[i].components[cp.length], ordValues.get(i), getCount(ordinals.get(i)));
    }
