 ...
 *   <li>{@link #newRangeQuery(String, BigInteger[], BigInteger[])} for matching points/ranges in
 *       n-dimensional space.
 * </ul>
 *
 * @see PointValues
...
public class BigIntegerPoint extends Field {
  ...
  public static final BigInteger MAX_VALUE =
      BigInteger.ONE.shiftLeft(BYTES * 8 - 1).subtract(BigInteger.ONE);
   ...
   * {@code lowerValue = BigIntegerPoint.MIN_VALUE} or {@code upperValue =
   * BigIntegerPoint.MAX_VALUE}.
   *
   * <p>Ranges are inclusive. For exclusive ranges, pass {@code lowerValue.add(BigInteger.ONE)} or
   * {@code upperValue.subtract(BigInteger.ONE)}
   *
   * @param field field name. must not be {@code null}.
   ...
   *
   * <p>You can have half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting
   * {@code lowerValue[i] = BigIntegerPoint.MIN_VALUE} or {@code upperValue[i] =
   * BigIntegerPoint.MAX_VALUE}.
   *
   * <p>Ranges are inclusive. For exclusive ranges, pass {@code lowerValue[i].add(BigInteger.ONE)}
