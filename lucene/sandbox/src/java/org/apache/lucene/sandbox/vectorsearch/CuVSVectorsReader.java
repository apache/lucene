...
public class CuVSVectorsReader extends KnnVectorsReader {
  ...
  public CuVSVectorsReader(SegmentReadState state, CuVSResources resources) throws Throwable {
    ...
    indexFilePayloadSize = (int)vectorDataReader.length() - (int)initialFilePointerLoc; //vectorMetaReader.readInt();
    segmentInputStream = new SegmentInputStream(vectorDataReader, indexFilePayloadSize, initialFilePointerLoc);
    ...
    log.info("Source of this segment "+segmentState.segmentSuffix+" is " + segmentState.segmentInfo.getDiagnostics().get(IndexWriter.SOURCE));
    log.info("Loading for " + segmentState.segmentInfo.name + ", mergeCase? " + isMergeCase);
    //if (!isMergeCase) { nocommit: TODO: don't load the cagra index for merge case.
  ...
  private Map<String, List<CuVSIndex>> loadCuVSIndex(ZipInputStream zis, boolean isMergeCase) throws Throwable {
    ...
    while ((ze = zis.getNextEntry()) != null) {
      ...
      switch (extension) {
        case "meta": {
          maxDocs = (Map<String, Integer>) SerializationUtils.deserialize(baos.toByteArray()); // nocommit use IOUtils
          break;
        }
        case "vec": {
          vectors.put(segmentField, (List<float[]>) SerializationUtils.deserialize(baos.toByteArray())); // nocommit use IOUtils
          break;
        }
        case "map": {
          List<Integer> map = (List<Integer>) SerializationUtils.deserialize(baos.toByteArray()); // nocommit use IOUtils
          mappings.put(segmentField, map);
          break;
        }
        case "cag": {
          cagraIndexes.put(segmentField, new CagraIndex.Builder(resources)
              .from(new ByteArrayInputStream(baos.toByteArray()))
              .build());
          break;
        }
        case "bf": {
          bruteforceIndexes.put(segmentField, new BruteForceIndex.Builder(resources)
              .from(new ByteArrayInputStream(baos.toByteArray()))
              .build());
          break;
        }
        case "hnsw": {
          HnswIndexParams indexParams = new HnswIndexParams.Builder(resources)
              .build();
          hnswIndexes.put(segmentField, new HnswIndex.Builder(resources)
              .from(new ByteArrayInputStream(baos.toByteArray()))
    ...
    for (String segmentField: cagraIndexes.keySet()) {
      ...
      CuVSIndex cuvsIndex = new CuVSIndex(segment, field, cagraIndexes.get(segmentField), mappings.get(segmentField), vectors.get(segmentField), maxDocs.get(segment), bruteforceIndexes.get(segmentField));
      List<CuVSIndex> listOfIndexes = ret.containsKey(field)? ret.get(field): new ArrayList<CuVSIndex>();
  ...
  public void search(String field, float[] target, KnnCollector knnCollector, Bits acceptDocs) throws IOException {
    PerLeafCuVSKnnCollector cuvsCollector = knnCollector instanceof PerLeafCuVSKnnCollector? ((PerLeafCuVSKnnCollector)knnCollector): new PerLeafCuVSKnnCollector(knnCollector.k(), knnCollector.k(), 1);
    TopKnnCollector defaultCollector = knnCollector instanceof TopKnnCollector? ((TopKnnCollector)knnCollector): null;

    int prevDocCount = 0;

    // log.debug("Will try to search all the indexes for segment "+segmentState.segmentInfo.name+", field "+field+": "+cuvsIndexes);
    for (CuVSIndex cuvsIndex: cuvsIndexes.get(field)) {
      try {
        ...
        if (cuvsCollector.k() <= 1024) {
          ...
          result = cagraIndex.search(query).getResults().get(0); // List expected to have only one entry because of single query "target".
          log.info("INTERMEDIATE RESULT FROM CUVS: " + result + ", prevDocCount=" + prevDocCount);
        } else {
          BruteForceQuery bruteforceQuery = new BruteForceQuery.Builder()
              .withQueryVectors(new float[][] { target })
