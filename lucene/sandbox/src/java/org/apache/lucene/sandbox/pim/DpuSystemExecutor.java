...
class DpuSystemExecutor implements PimQueriesExecutor {
   ...
   *     segment id
   * @return a positive integer with the size of the buffer needed if results is too small to contain
   * all results. In this case, no transfer has been done and the API should be called again with a buffer
   * of the right size. If the buffer was big enough, the return value is zero
   */
   ...
   */
  native void runDpusWithLowerBound(int nr_queries, int[] numHits, int[] quantFactors,
                           Similarity.SimScorer[] scorers);
