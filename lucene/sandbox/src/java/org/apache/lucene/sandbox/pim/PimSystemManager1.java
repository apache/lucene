...
public final class PimSystemManager1 implements PimSystemManager {
  ...
  public boolean isQuerySupported(Query query) {
    ...
    if (query instanceof PimPhraseQuery) return true;
    else return false;
  }

  /** Kill the thread created by this singleton to handle the interface with the PIM HW */
  ...
  public <QueryType extends Query & PimQuery> DpuResultsReader search(
      LeafReaderContext context, QueryType query) throws PimQueryQueueFullException {

    if (!isQuerySupported(query)) return null;
    ...
    try {
      ...
      queryLock.lock();
      try {
        if (DEBUG) System.out.println("Signal query");
        queryPushedCond.signal();
      } finally {
        queryLock.unlock();
      }
      if (DEBUG) System.out.println("Waiting for result");
      ...
      try {
        while (!queryResultsAvailable(id)) resultsPushedCond.await();
      ...
      if (DEBUG) System.out.println("Reading result");
  ...
  /**
   * Returns the results of a query by interpreting data returned by the PIM system and creating a
   * list of PimMatch objects.
   *
   ...
   * @throws IOException
   */
  private <QueryType extends Query & PimQuery> DpuResultsReader getQueryMatches(int id)
      throws IOException {
  ...
  /**
   * Register a query id in the map of processed queries. This is used to avoid query id clashes
   * when a query is out of the query queue but its results not yet processed.
   *
   * @param id
   */
  ...
  private <QueryType extends Query & PimQuery> void writeQueryToPim(
      DataOutput output, QueryType query, int leafIdx) throws IOException {
  ...
  /**
   * PIM Manager thread This thread is responsible for regularly checking the query input queue,
   * executing a batch of queries, and pushing the results in a TreeMap.
   */
      ...
      while (running) {
        ...
        try {
          ...
          while (slice.getNbElems() == 0) {
            ...
            try {
              ...
              if (slice.getNbElems() == 0) {
                queryBuffer.release();
                if (DEBUG) System.out.println("Waiting for query");
          ...
          if (WAIT_FOR_BATCH_NS != 0 && slice.getNbElems() < NUM_QUERIES_MIN) {
            queryBuffer.release();
            if (DEBUG) System.out.println("Handling query but waiting a bit more");
            Thread.sleep(0, WAIT_FOR_BATCH_NS);
            slice = queryBuffer.peekMany(QUERY_BATCH_SIZE);
          }
          if (DEBUG) System.out.println("Handling query");
          ...
          try {
            ...
            if (DEBUG) System.out.println("Signal results, nb res:" + queryResultsMap.size());
