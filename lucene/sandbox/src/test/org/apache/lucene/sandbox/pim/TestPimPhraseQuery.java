...
@TestRuleLimitSysouts.Limit(bytes = 1 << 16, hardLimit = 1 << 16)
public class TestPimPhraseQuery extends LuceneTestCase {
  ...
  private static RandomIndexWriter.IndexWriterInstanceCreator indexCreator;
  private static PimConfig pimConfig;
  private static final int nbDpus = 32;
  private static final boolean pimMode = true;

  @BeforeClass
  public static void beforeClass() throws Exception {
    directory = newDirectory();
    pimDirectory = newDirectory();
    pimConfig = new PimConfig(nbDpus);
    if (pimMode) indexCreator = getIndexCreator(directory, pimDirectory, pimConfig);
    else indexCreator = getIndexCreator(directory);
    ...
    RandomIndexWriter writer = new RandomIndexWriter(random(), directory, analyzer, indexCreator);
    ...
    if (pimMode) assert PimSystemManager.get().loadPimIndex(pimDirectory);
  }
  ...
  public void testOrderDoesntMatter() throws Exception {
    ...
    query = new PimPhraseQuery(2, "field", "two", "one");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PimPhraseQuery(2, "field", "three", "one");
    hits = searcher.search(query, 1000).scoreDocs;
  ...
  public void testMultipleTerms() throws Exception {
    query = new PimPhraseQuery(2, "field", "one", "three", "five");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("two total moves", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    // it takes six moves to match this phrase
    query = new PimPhraseQuery(5, "field", "five", "three", "one");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("slop of 5 not close enough", 0, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PimPhraseQuery(6, "field", "five", "three", "one");
    hits = searcher.search(query, 1000).scoreDocs;
  ...
  public void testPhraseQueryWithStopAnalyzer() throws Exception {
    ...
    Directory pimDir;
    RandomIndexWriter.IndexWriterInstanceCreator indexCreator;
    if (pimMode) {
      pimDir = newDirectory();
      indexCreator = getIndexCreator(directory, pimDir, pimConfig);
    } else indexCreator = getIndexCreator(directory);
    ...
    RandomIndexWriter writer =
        new RandomIndexWriter(
            random(), directory, newIndexWriterConfig(stopAnalyzer), indexCreator);
    Document doc = new Document();
    ...
    PhraseQuery query = new PimPhraseQuery("field", "stop", "words");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals(1, hits.length);
    QueryUtils.check(random(), query, searcher);
  ...
  private static RandomIndexWriter.IndexWriterInstanceCreator getIndexCreator(Directory dir) {
    return (conf) -> {
  ...
  private static RandomIndexWriter.IndexWriterInstanceCreator getIndexCreator(
      Directory dir, Directory pimDir, PimConfig pimConf) {
    return (conf) -> {
      return new PimIndexWriter(dir, pimDir, conf, pimConf) {
        @Override
        protected boolean isEnableTestPoints() {
          return true;
        }
  ...
  public void testPhraseQueryInConjunctionScorer() throws Exception {
    ...
    RandomIndexWriter writer = new RandomIndexWriter(random(), directory, indexCreator);
    ...
    phraseQuery = new PimPhraseQuery("contents", "map", "entry");
  ...
  public void testSlopScoring() throws IOException {
    ...
    PhraseQuery query = new PimPhraseQuery(Integer.MAX_VALUE, "field", "firstname", "lastname");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals(3, hits.length);
  ...
  public void testToString() throws Exception {
    PhraseQuery q = new PimPhraseQuery("field", new String[0]);
    assertEquals("\"\"", q.toString());

    PhraseQuery.Builder builder = new PimPhraseQuery.Builder();
    builder.add(new Term("field", "hi"), 1);
    q = builder.build();
    assertEquals("field:\"? hi\"", q.toString());

    builder = new PimPhraseQuery.Builder();
    builder.add(new Term("field", "hi"), 1);
  ...
  public void testWrappedPhrase() throws IOException {
    query = new PimPhraseQuery(100, "repeated", "first", "part", "second", "part");

    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("slop of 100 just right", 1, hits.length);
    QueryUtils.check(random(), query, searcher);

    query = new PimPhraseQuery(99, "repeated", "first", "part", "second", "part");
  ...
  public void testNonExistingPhrase() throws IOException {
    ...
    query = new PimPhraseQuery(2, "nonexist", "phrase", "notexist", "found");
    ...
    query = new PimPhraseQuery(1, "nonexist", "phrase", "exist", "exist");
    ...
    query = new PimPhraseQuery(1000, "nonexist", "phrase", "notexist", "phrase");
    ...
    query = new PimPhraseQuery(1000, "nonexist", "phrase", "exist", "exist", "exist");
  ...
  public void testPalyndrome2() throws Exception {
    ...
    query = new PimPhraseQuery("field", "two", "three"); // to use exact phrase scorer
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    ...
    query = new PimPhraseQuery("field", "two", "three"); // to use sloppy scorer
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
  ...
  public void testPalyndrome3() throws Exception {
    ...
    query = new PimPhraseQuery(0, "field", "one", "two", "three");
    ScoreDoc[] hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("phrase found with exact phrase scorer", 1, hits.length);
    float score0 = hits[0].score;
    ...
    query = new PimPhraseQuery(4, "field", "one", "two", "three");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
    float score1 = hits[0].score;
    ...
    query = new PimPhraseQuery(4, "palindrome", "one", "two", "three");
    hits = searcher.search(query, 1000).scoreDocs;
    ...
    query = new PimPhraseQuery(4, "palindrome", "three", "two", "one");
    hits = searcher.search(query, 1000).scoreDocs;
    assertEquals("just sloppy enough", 1, hits.length);
    // float score3 = hits[0].score;
  ...
  public void testEmptyPhraseQuery() throws Throwable {
    ...
    q2.add(new PimPhraseQuery("field", new String[0]), BooleanClause.Occur.MUST);
    q2.build().toString();
  }

  /* test that a single term is rewritten to a term query */
  public void testRewrite() throws IOException {
    PhraseQuery pq = new PimPhraseQuery("foo", "bar");
  ...
  public void testZeroPosIncr() throws IOException {
    ...
    if (pimMode) {
      pimDir = newDirectory();
      indexCreator = getIndexCreator(dir, pimDir, pimConfig);
    } else indexCreator = getIndexCreator(dir);
    ...
    RandomIndexWriter writer = new RandomIndexWriter(random(), dir, indexCreator);
    Document doc = new Document();
  ...
  public void testNegativeSlop() throws Exception {
    expectThrows(
        ...
        () -> {
          new PimPhraseQuery(-2, "field", "two", "one");
        });
  }
  ...
  public void testTopPhrases() throws IOException {
    ...
    for (String value : DOCS) {
      Document doc = new Document();
      doc.add(new TextField("f", value, Field.Store.NO));
      w.addDocument(doc);
    }
    ...
    for (Query query :
        Arrays.asList(
            new PimPhraseQuery("f", "b", "c"), // common phrase
            new PimPhraseQuery("f", "e", "f"), // always appear next to each other
            new PimPhraseQuery("f", "d", "d") // repeated term
            )) {
  ...
  public void testRandomTopDocs() throws IOException {
    ...
    for (int i = 0; i < numDocs; ++i) {
      ...
      doc.add(new TextField("foo", text, Field.Store.NO));
      w.addDocument(doc);
    }
    ...
    for (String firstTerm : new String[] {"a", "b", "c"}) {
      for (String secondTerm : new String[] {"a", "b", "c"}) {
        Query query = new PimPhraseQuery("foo", newBytesRef(firstTerm), newBytesRef(secondTerm));
        ...
        Query filteredQuery =
            new BooleanQuery.Builder()
                ...
                .add(new TermQuery(new Term("foo", "b")), BooleanClause.Occur.FILTER)
                .build();
  ...
  public void testNullTerm() {
    NullPointerException e =
        expectThrows(NullPointerException.class, () -> new PimPhraseQuery.Builder().add(null));
    assertEquals("Cannot add a null term to PhraseQuery", e.getMessage());

    e =
        expectThrows(
            NullPointerException.class, () -> new PimPhraseQuery("field", (BytesRef) null));
    assertEquals("Cannot add a null term to PhraseQuery", e.getMessage());

    e = expectThrows(NullPointerException.class, () -> new PimPhraseQuery("field", (String) null));
    assertEquals("Cannot add a null term to PhraseQuery", e.getMessage());
  }
}
