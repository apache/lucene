 ...
 * @lucene.experimental
 */
class GlobalOrdinalsCollectorManager
    implements CollectorManager<
        GlobalOrdinalsCollectorManager.GlobalOrdinalsCollector, LongBitSet> {
  ...
  public GlobalOrdinalsCollector newCollector() throws IOException {
    return new GlobalOrdinalsCollector();
  }

  @Override
  public LongBitSet reduce(Collection<GlobalOrdinalsCollector> collectors) throws IOException {
    LongBitSet combined = new LongBitSet(valueCount);
    for (GlobalOrdinalsCollector collector : collectors) {
      combined.or(collector.collectedOrds);
    }
    return combined;
  }

  final class GlobalOrdinalsCollector implements Collector {
    private final LongBitSet collectedOrds = new LongBitSet(valueCount);
