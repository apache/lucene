 ...
 * on which cells are within the query shape. It exposes a {@link TwoPhaseIterator} that will verify
 * a match with a provided predicate in the form of an ShapeValuesPredicate.
 *
 * @lucene.internal
 */
public class IntersectsRPTVerifyQuery extends Query {
  ...
  public IntersectsRPTVerifyQuery(
      Shape queryShape,
      String fieldName,
    ...
    this.intersectsDiffQuery =
        new IntersectsDifferentiatingQuery(
  ...
  // This may be a "Query" but we don't use it as-such; the caller calls the constructor and then
  // compute() and examines
  // the results which consists of two parts -- the approximated results, and a subset of exact
  // matches. The
  ...
  private static class IntersectsDifferentiatingQuery extends AbstractVisitingPrefixTreeQuery {
    ...
    public IntersectsDifferentiatingQuery(
        Shape queryShape,
        String fieldName,
        SpatialPrefixTree grid,
        int detailLevel,
    ...
    IntersectsDifferentiatingQuery.IntersectsDifferentiatingVisitor compute(
