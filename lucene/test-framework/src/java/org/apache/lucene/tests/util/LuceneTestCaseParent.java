...
/**
 * Private parent class for junit4 ({@link LuceneTestCase} and junit jupiter ({@link
...
public abstract sealed class LuceneTestCaseParent extends Assert
   ...
   */
  protected static IndexSearcher.LeafSlice[] slices(
      List<LeafReaderContext> leaves,
      int maxDocsPerSlice,
      int maxSegmentsPerSlice,
      LuceneTestCase.Concurrency concurrency) {
  ...
  private static final StackWalker
      SW_NO_METHODS = StackWalker.getInstance(StackWalker.Option.DROP_METHOD_INFO),
  ...
  public static void maybeChangeLiveIndexWriterConfig(Random r, LiveIndexWriterConfig c) {
    ...
    if (rarely(r)) {
      ...
      synchronized (c) {
        boolean flushByRAM;
        switch (getTestFrameworkInfra().getClassEnv().getLiveIWCFlushMode()) {
          case BY_RAM:
            flushByRAM = true;
            break;
          case BY_DOCS:
  ...
  public static Field newStringField(String name, String value, Field.Store stored) {
    return newField(
        ...
        name,
        value,
        stored == Field.Store.YES ? StringField.TYPE_STORED : StringField.TYPE_NOT_STORED);
  }
  ...
  public static Field newTextField(String name, String value, Field.Store stored) {
    return newField(
        ...
        name,
        value,
        stored == Field.Store.YES ? TextField.TYPE_STORED : TextField.TYPE_NOT_STORED);
  }
  ...
  public static Field newStringField(
      Random random, String name, BytesRef value, Field.Store stored) {
    return newField(
        random,
        name,
        value,
