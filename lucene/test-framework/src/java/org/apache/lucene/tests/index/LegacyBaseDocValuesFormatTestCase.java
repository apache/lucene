...
public abstract class LegacyBaseDocValuesFormatTestCase extends BaseIndexFileFormatTestCase {
  ...
  public void testThreeFieldsMixed() throws IOException {
    ...
    TopDocs hits = isearcher.search(query, 1);
    StoredFields storedFields = isearcher.storedFields();
<<<<<<<< HEAD:lucene/test-framework/src/java/org/apache/lucene/tests/index/BaseDocValuesFormatTestCase.java
    assertEquals(1, hits.totalHits.value);
========
    assertEquals(1, hits.totalHits.value());
>>>>>>>> origin/branch_10_1:lucene/test-framework/src/java/org/apache/lucene/tests/index/LegacyBaseDocValuesFormatTestCase.java
    // Iterate through the results:
    for (int i = 0; i < hits.scoreDocs.length; i++) {
      int docID = hits.scoreDocs[i].doc;
      Document hitDoc = storedFields.document(docID);
