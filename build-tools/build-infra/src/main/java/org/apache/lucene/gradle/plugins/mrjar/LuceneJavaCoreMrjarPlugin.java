...
public class LuceneJavaCoreMrjarPlugin extends LuceneGradlePlugin {
  /** Classpath configuration for the ASM-based {@link ExtractJdkApis} utility. */
  private static final String APIEXTRACTOR_CONFIGURATION = "apiextractor";
  ...
  static void setupMrJarInfrastructure(Project project, List<Integer> mrjarJavaVersions) {
    ...
    // Classpath for the ASM-based api extractor utility.
    project.getConfigurations().create(APIEXTRACTOR_CONFIGURATION);
    project
        .getDependencies()
        .add(
            APIEXTRACTOR_CONFIGURATION,
            getVersionCatalog(project).findLibrary("asm-core").orElseThrow());
      ...
      var generateJdkApiJarTask =
          tasks.register(
              ...
              spec -> {
                ...
                // The extractor uses ASM (the classfile API is not available on the JDKs
                // this branch extracts stubs from), so run it with the asm dependency
                // on the classpath.
                spec.setClasspath(
                    project.getConfigurations().getByName(APIEXTRACTOR_CONFIGURATION));
                ...
                spec.setArgs(List.of(jdkVersion.toString(), apijar.getAsFile().toString()));
              });
