...
def main():
    ...
    if transpose:
        w(
            "/** Parametric description for generating a Levenshtein automaton of degree %s, "
            % n
        )
    ...
    if not transpose and n == 1:
        ...
        w(
            "   The comment in Lev1ParametricDescription may be helpful for you to understand this class."
        )
    ...
    for i, map in enumerate(tables):
        ...
        numVectors = len(items)

        if MODE == "array":
            toStateArray = []
            toOffsetIncrArray = []

        for charVar, states in items:
            ...
            if i != 0 and MODE == "switch":
                w(
                    "case %s: // <%s>"
                    % (charVarNumber(charVar), ",".join([str(x) for x in charVar]))
                )
                w.indent()
            ...
            if numCasesPerVector is None:
                numCasesPerVector = len(items)
            else:
                # we require this to be uniform... empirically it seems to be!
                assert numCasesPerVector == len(items)
            ...
            if MODE == "array":
                ...
                # render switches
                w("switch(state) {   // %s cases" % len(items))
        ...
        machines.append(
            (toStateArray, toOffsetIncrArray, numCasesPerVector, numVectors)
        )
    ...
    if MODE == "array":
        ...
        for i, (
            ...
            if transpose or n == 2:
                w(
                    "// %d vectors; %d states per vector; array length = %d"
            ...
            if PACKED:
                # pack in python
                items, nbits = pack(toStateArray)
                subs.append(("NBITSSTATES%d" % i, str(nbits)))
                w(
                    "  private final static long[] toStates%d = new long[] /*%d bits per value */ %s;"
                    % (i, nbits, renderList([(hex(int(x)) + "L") for x in items]))
                )

                items, nbits = pack(toOffsetIncrsArray)
                subs.append(("NBITSOFFSET%d" % i, str(nbits)))
                w(
                    "  private final static long[] offsetIncrs%d = new long[] /*%d bits per value */ %s;"
                    % (i, nbits, renderList([(hex(int(x)) + "L") for x in items]))
                )
            else:
                w(
                    "  private final static int[] toStates%d = new int[] %s;"
                    % (i, renderList([str(x) for x in toStateArray]))
                )
                w(
                    "  private final static int[] offsetIncrs%d = new int[] %s;"
                    % (i, renderList([str(x) for x in toStateArray]))
                )
        w.outdent()
    ...
    w(
        "    super(w, %d, new int[] {%s});"
        % (n, ",".join([str(x) for x in minErrors])),
        indent=1,
    )
    w("  }")
    ...
    if MODE == "array" and PACKED:
        ...
        if False:
            ...
            items = []
            for i in range(63):
                items.append(hex(v - 1))
                v *= 2

            w(
                "private final static long[] MASKS = new long[] {%s};"
                % ",".join(items),
                indent=1,
            )
            w("")
    ...
    print(
        "Wrote %s [%d lines; %.1f KB]"
        % (fileOut, len(w.l), os.path.getsize(fileOut) / 1024.0)
    )
...
def pack(items):
    ...
    for i in range(len(items)):
        ...
        if pendingValue > 0:
            ...
            assert bitsUsed <= (WORD - bitsLeft), (
                "bitsLeft=%s (%s-%s=%s) bitsUsed=%s"
                % (bitsLeft, WORD, bitsLeft, WORD - bitsLeft, bitsUsed)
            )
